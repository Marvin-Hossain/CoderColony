...
public interface TechnicalQuestionRepository extends JpaRepository<TechnicalQuestion, Long> {
    ...
    @Query(value = "SELECT * FROM technical_questions WHERE question NOT IN " +
           "(SELECT question FROM technical_questions WHERE created_at = CURRENT_DATE AND response_text IS NOT NULL) " +
